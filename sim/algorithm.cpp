...
void delay(int time_milliseconds) {
  sleep(time_milliseconds / 100);
}
...
float readSensorData(){
  ...
  */
  float distance = 999; //TODO lookup from the sim (HOW?)
  return distance;
}
...
bool isThereAWallInFrontOfMe(int distance) {
  ...
  print(" cm.  <-- ultrasonic sensor reading");
  ...
  */
  ...
  if (adjacentCell >= 0 && adjacentCell < 16) {
    ...
    if (!invalidEdge) {
      ...
      print("Updated adjacent cell ");
      print(to_string(adjacentCell));
      print(" wall info: ");
      //print(adjacentCellData.wallInfo, BIN);
    }
  }
  return;
}
...
int main() {
  ...
  if (loopCount == 0) {
    print("First loop. Clearing LCD in 3 seconds.");
    delay(2900);
    //lcd.clear();
    loopCount = 1;
    //lcd.setCursor(2, 0);
